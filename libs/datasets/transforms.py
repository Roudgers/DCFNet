...
class RandomCrop(object):   # python中所有函数都是可调用对象，一个类的实例也可以变成一个可调用对象，通过def __call__
    def __init__(self, size, padding=0):    # def __init__创建一个类的实例
        if isinstance(size, numbers.Number):    # 判断是否 size 属于 Number类型
    ...
    def __call__(self, sample):
        ...
        assert label.size == label.size    # 表达式若为真，程序继续往下进行
        w, h = image.size
